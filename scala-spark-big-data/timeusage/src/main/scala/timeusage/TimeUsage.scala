...
object TimeUsage {
  ...
  def classifiedColumns(
      columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    val results =
      (0 to 2).map(index => groups.getOrElse(index, List.empty[Column]))
  ...
  def timeUsageSummary(
    ...
    val workingStatusProjection: Column = when(
      ($"telfs" < 3).&&($"telfs" > 0),
      "working").otherwise("not working").as("working")
    ...
    df.select(workingStatusProjection,
              sexProjection,
              ageProjection,
  ...
  def timeUsageSummaryTyped(
      timeUsageSummaryDf: DataFrame): Dataset[TimeUsageRow] =
  ...
  def timeUsageGroupedTyped(
      summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {

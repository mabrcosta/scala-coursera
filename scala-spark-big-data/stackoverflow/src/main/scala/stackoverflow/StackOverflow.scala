...
class StackOverflow extends Serializable {
  ...
  def groupedPostings(
      postings: RDD[Posting]): RDD[(QID, Iterable[(Question, Answer)])] = {
    val questions: RDD[(QID, Posting)] =
      postings.filter(_.postingType == 1).map(v => (v.id, v))
    val answers: RDD[(QID, Posting)] =
      postings.filter(_.postingType == 2).map(v => (v.parentId.get, v))
  ...
  def scoredPostings(grouped: RDD[(QID, Iterable[(Question, Answer)])])
    ...
    grouped.map(a => (a._2.head._1, answerHighScore(a._2.map(_._2).toArray)))
  }

  /** Compute the vectors for the kmeans */
  def vectorPostings(
      scored: RDD[(Question, HighScore)]): RDD[(LangIndex, HighScore)] = {
    ...
    scored
      .map(v => (firstLangInTag(v._1.tags, langs), v._2))
    ...
    val res =
      if (langSpread < 500)
        ...
        vectors.groupByKey
          .flatMap({
            case (lang, vectors) =>
              reservoirSampling(lang, vectors.toIterator, perLang).map(
                (lang, _))
          })
          .collect()
  ...
  //
  //
  type KernelPoint = Int

  def clusterResults(means: Array[(Int, Int)],
                     vectors: RDD[(LangIndex, HighScore)])
    : Array[(String, Double, Int, Int)] = {
    val closest = vectors.map(p => (findClosest(p, means), p))
    val closestGrouped: RDD[(KernelPoint, Iterable[(LangIndex, HighScore)])] =
      closest.groupByKey()

    val median = closestGrouped.mapValues { vs =>
      val langsCount = vs.groupBy(_._1).mapValues(_.size)
      val maxLangIndex: LangIndex = langsCount.toSeq.maxBy(_._2)._1

      val langLabel: String = langs(maxLangIndex / langSpread) // most common language in the cluster
      val clusterSize: Int = vs.size
      val langPercent: Double = (langsCount(maxLangIndex).toDouble / clusterSize) * 100 // percent of the questions in the most common language
      val medianScore: Int = findMedian(vs.map(_._2).toSeq)
  ...
  def findMedian(values: Seq[HighScore]): HighScore = {
    val sortedScores = values.sorted
    val size = values.size
    val middle = size / 2
    if(size % 2 == 0) (sortedScores(middle - 1) + sortedScores(middle)) / 2 else sortedScores(middle)
  }

...
import FloatOps._
  ...
  test("Fork with 4 empty quadrants") {
    ...
    val se = Empty(22.5f, 32.5f, 5f)
    val quad = Fork(nw, ne, sw, se)
    val updtQuad = quad.insert(b)

    assert(updtQuad.centerX == 20f, s"${updtQuad.centerX} should be 20f")
    assert(updtQuad.centerY == 30f, s"${updtQuad.centerY} should be 30f")
    assert(updtQuad.mass ~= 123f, s"${updtQuad.mass} should be 123f")
    assert(updtQuad.massX ~= 18f, s"${updtQuad.massX} should be 18f")
    assert(updtQuad.massY ~= 26f, s"${updtQuad.massY} should be 26f")
    assert(updtQuad.total == 1, s"${updtQuad.total} should be 1")
  }

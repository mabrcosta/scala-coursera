...
class ParallelParenthesesBalancingSuite extends FunSuite {
  ...
  test("balance should work for sequence of parentheses without nesting") {
    def check(input: String, expected: Boolean) =
      assert(balance(input.toArray) == expected,
        s"balance($input) should be $expected")
...
//  }
  ...
  test("parBalance should work for string of length 100000000") {
    def check(input: Array[Char], expected: Boolean) =
      assert(parBalance(input, 10000) == expected,
        s"parBalance($input) should be $expected")

    val chars = new Array[Char](100000000)
    chars(0) = '('
    chars(1) = ')'

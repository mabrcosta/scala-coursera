...
class LineOfSightSuite extends FunSuite {
  ...
  test("upsweep should correctly compute the tree on the indices 1 until 5 of a 5 element array for threshold 1") {
    val res = upsweep(Array[Float](0f, 1f, 8f, 9f, 9f), 1, 5, 1)
    assert(true)
  }

  test("parLineOfSight should correctly compute the output for threshold 2") {
    val output = new Array[Float](5)
    parLineOfSight(Array[Float](0f, 7f, 10f, 36f, 9f), output, 2)
    assert(output.toList == List(0f, 7f, 7f, 12f, 12f))
  }
  ...
  test("parLineOfSight should invoke the parallel construct 30 times (15 times during upsweep and 15 times during downsweep) for an array of size 17, with threshold 1") {
    ...
    parLineOfSight(Array[Float](0f, 7f, 10f, 36f, 9f), output, 1)
    assert(output.toList == List(0f, 7f, 7f, 12f, 12f))
  }

...
class BlurSuite extends FunSuite {
  ...
  test("boxBlurKernel should return the correct value on an border pixel " +
    ...
    assert(boxBlurKernel(src, 0, 0, 1) === 2,
      s"(boxBlurKernel(0, 0, 1) should be 12, " +
  ...
  test("HorizontalBoxBlur.parBlur with radius 1 should correctly blur the entire 3x3 image") {
    ...
    check(1, 1, 4)
    check(2, 1, 4)
//    check(0, 2, 0) //On previous test they were not borred
//    check(1, 2, 0)
//    check(2, 2, 0)
  }
  ...
  test("VerticalBoxBlur.blur with 1920 x 1080 image") {
    val w = 1920
    val h = 1080
    val src = new Img(w, h)
    val dst = new Img(w, h)

    VerticalBoxBlur.blur(src, dst, 0, w, 2)
  }

  test("VerticalBoxBlur.parblur with 1920 x 1080 image") {
    val w = 32
    val h = 64
    val src = new Img(w, h)
    val dst = new Img(w, h)

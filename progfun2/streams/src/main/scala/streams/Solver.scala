...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    if (initial.isEmpty) initial
      ...
      if (newNeighbours.isEmpty) initial append from(initial.tail, explored + initial.head._1)
      else {
        val updated = initial append newNeighbours
        updated append from(updated.tail, explored + updated.head._1)
      }
    }
  }
